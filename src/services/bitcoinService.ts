...
class BitcoinService {
  ...
  async createPaymentRequest(
    ...
    amountUSD: number,
    planName: string = 'Savings Deposit'
  ) {
    try {
      ...
      if (planId) {
        paymentData.planId = new mongoose.Types.ObjectId(planId);
      }
   ...
   */
  private async getBTCExchangeRate(): Promise<number> {
  ...
  try {
    ...
    if (this.btcExchangeRate && this.btcExchangeRate > 0) {
      console.warn(
        `[BITCOIN] Using cached BTC exchange rate: $${this.btcExchangeRate} USD`
      );
      return this.btcExchangeRate;
    }
    ...
    console.warn(
      `[BITCOIN] Using fallback BTC exchange rate: $${fallbackRate} USD`
    );
  ...
  try {
    console.log(
      `[BITCOIN] Fetching transaction details for hash: ${transactionHash}`
    );

    const response = await fetch(
      `https://blockstream.info/api/tx/${transactionHash}`,
      {
        signal: controller.signal,
      }
    );
    ...
    if (!response.ok) {
      throw new Error(
        `Blockstream API returned ${response.status}: Transaction not found or invalid hash`
      );
    }
    ...
    console.log(
      `[BITCOIN] Transaction found. Outputs: ${tx.vout?.length || 0}`
    );
    console.log(
      `[BITCOIN] Looking for outputs to address: ${bitcoinAddress}`
    );
    ...
    if (Array.isArray(tx.vout)) {
      ...
      for (const output of tx.vout) {
        ...
        console.log(
          `[BITCOIN] Output address: ${outputAddress}, amount: ${output.value} satoshis`
        );
        ...
        if (
          ...
          console.log(
            `[BITCOIN] ✓ Found matching output of ${output.value} satoshis`
          );
        }
      }
    }
    ...
    if (totalAmountToUs === 0) {
      console.warn(
        `[BITCOIN] ⚠️ No outputs found for address ${bitcoinAddress}`
      );
      console.warn(`[BITCOIN] Transaction hash: ${transactionHash}`);
    }

    // Convert satoshis to BTC
    const amountBTC = totalAmountToUs / 100_000_000;

    console.log(
      `[BITCOIN] Amount sent to ${bitcoinAddress}: ${amountBTC} BTC (${totalAmountToUs} satoshis)`
    );
    ...
    if (error.name === "AbortError") {
      console.error(
        "[BITCOIN] Blockstream request timed out after 10 seconds."
      );
    } else {
      console.error(
        "[BITCOIN] Error fetching transaction amount:",
        error
      );
    }
  ...
  async verifyPaymentByReference(
    ...
    try {
      ...
      if (existingCompletedPayment) {
        ...
        throw new Error(
          `This Bitcoin transaction hash has already been used for payment. ` +
      ...
      if (actualUSD < expectedUSD) {
        ...
        throw new Error(
          `Bitcoin amount is insufficient! You sent ${actualBTC} BTC (${actualUSD.toFixed(2)} USD) but the investment requires ${expectedUSD} USD (${expectedBTC} BTC). ` +
   ...
   */
  async completePaymentAndCreateInvestment(
    paymentReference: string,
   ...
   */
  async completePaymentAndDepositSavings(
    paymentReference: string,
    savingsService: any
  ): Promise<any> {
    try {
      ...
      const savings = await savingsService.completeSavingsDeposit(
        payment.userId,
        payment.amount,
        payment._id.toString()
      );
