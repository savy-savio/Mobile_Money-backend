...
class DailyGrowthJob {
   ...
   */
  static async processInvestmentGrowth(investment: any): Promise<void> {
    try {
      // Get the investment plan details
      const plan = await InvestmentPlan.findById(investment.planId);
      if (!plan) {
        console.log(
          `[DAILY-GROWTH-JOB] Plan not found for investment ${investment._id}`
        );
        return;
      }
      ...
      const growthRecord = new InvestmentGrowth({
      ...
      console.log(
        `[DAILY-GROWTH-JOB] Investment ${investment._id}: ${previousValue.toFixed(2)} → ${newValue.toFixed(2)} (+$${dailyGain.toFixed(2)})`
      );
    } catch (error) {
      console.error(
        `[DAILY-GROWTH-JOB] Error processing investment ${investment._id}:`,
        error
      );
